...
def sobolev_kernel(x, y, l=1., s=None):
	"""
	...
	### If s > (d + 1) / 2 we may get warnings due to the hack that handles x = y
		...
		res[arg !=0.] = (c * arg**(s - dim/2) * kv(s - dim/2, arg))[arg!=0.]
	return res
...
def make_kernels(X, Y, X_fill, Y_fill, l=.1, kernel='gaussian', product_sampling=False, **kwargs):
	"""
...
def transport_cost(G, Kx2, Kx3, Ky2, Ky3, lbda_2, product_sampling=False):
	...
	if product_sampling:
		Gam = G.reshape(n, m)
		return (Kx3.mean() + Ky3.mean() - (Kx2 @ Gam).sum() / n  - (Gam @ Ky2).sum() / m) / (2 * lbda_2)
	else:
		return  (Kx3.mean() + Ky3.mean() - (G * Kx2.mean(0)).sum()  - (G * Ky2.mean(0)).sum()) / (2 * lbda_2)

def sobolev_grad(x, y, l=1., s=None):
	"""
	...
	### If s > (d + 1) / 2 we may get warnings due to the hack that handles x = y
		...
		c = 2**(1 + dim/2 - s) / gamma(s - dim/2) ## If l != 1, is this the right constant?
		res[idxs] = c * ((s - dim/2) / (l**(s - dim /2)) * (x-y) * arg**(s - dim/2 - 2) * kv(s - dim/2, arg / l)
							 + (x-y) / (l**(s - dim /2 + 1))* arg**(s-dim/2 - 1) * kvp(s - dim/2, arg / l))[idxs]
	return res.squeeze()


def gaussian_grad(x, y, l=1.):
	"""
	...
	"""
	if len(x.shape) == 1:
		return (-2. / l * (x - y) * gaussian_kernel(x, y, l)[:, None]).squeeze()
	else:
		return (-2. / l * (x - y) * gaussian_kernel(x, y, l)[:, :, None]).squeeze()


def potential_1D(x, G, X, X_fill, lbda, l=1., kernel = 'gaussian', **kwargs):
	"""
	...
	**kwargs
	...
	if kernel == 'sobolev':
		...
		kernel_func = gaussian_kernel
	if len(x.shape) == 1:
		return 1 / (2 * lbda) * (1 / n * kernel_func(x, X, l=l, **kwargs).sum() \
				- (G  * kernel_func(x, X_fill, l=l, **kwargs)).sum(0))

	else:
		return 1 / (2 * lbda) * ((1 / n * kernel_func(x[:, None], X[:, None], l=l, **kwargs)).sum(1) \
				- (G * kernel_func(x[:, None], X_fill, l=l, **kwargs)).sum(1))



def transport_1D(x, G, X, X_fill, lbda_2, l=1., kernel='gaussian', **kwargs):
	"""
	...
	**kwargs
	...
	return x - (1. / n * grad_func(x[:, None, None], X[:, None], l, **kwargs).sum(1) - (G * grad_func(x[:, None, None], X_fill, l, **kwargs)).sum(1)) / (2 * lbda_2)
